...
export class Paginator {
    ...
    private async init(): Promise<void> {
        this.message = await this.channel.send(
            this.embed.setDescription(this.pages[this.curPage]).setFooter(`Page ${this.curPage + 1} of ${this.pages.length}`),
        );
        ...
        this.collector = this.message.createReactionCollector(
            (reaction, user) => reaction.me && user.id === this.member.id && user.id !== this.message.author!.id,
            { time: this.timeout },
        );
        ...
        this.collector.on('collect', async (reaction: MessageReaction, user: User) => {
            ...
            switch (reaction.emoji.toString()) {
                ...
                case emojis.back:
                    this.curPage--;
                    if (this.curPage < 0) this.curPage = this.pages.length - 1;
                    break;
                case emojis.next:
                    this.curPage++;
    ...
    private async refresh(): Promise<void> {
        await this.message.edit(this.embed.setDescription(this.pages[this.curPage]).setFooter(`Page ${this.curPage + 1} of ${this.pages.length}`));
    }
}
